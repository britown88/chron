...
int deviceContextListFiles(const char *root, int type, vec(StringPtr) **out, const char *ext) {
   ...
   StringCchLength(root, MAX_PATH, (uPtr*)&length_of_arg);
   ...
   *out = vecCreate(StringPtr)(&stringPtrDestroy);
   ...
   do {
      ...
      if (type == DC_FILE_ALL ||
         ...
         if (ext) {
            u32 len = (u32)strlen(ffd.cFileName);
            u32 dotPos;
            char *str;
