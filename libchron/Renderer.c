...
static u32 _s2uHash(StringToUniform *p) {
   u32 out = 5031;
...
static unsigned int _shaderCompile(Shader *self, vec(StringPtr) *lines, int type) {
   ...
   if (handle) {
      int i = 0;
      int compileStatus;
      u32 lineCount = vecSize(StringPtr)(lines);
...
static u32 _texEntryHash(TexturePtr *p) {
   u32 h = 5381;
...
void uboSetData(UBO *self, u32 offset, u32 size, void *data) {
   if (!self->built) {
      _uboBuild(self);
   }
...
static uPtr _vertexAttributeByteSize(VertexAttribute attr) {
   switch (attr) {
...
Model *__modelCreate(void *data, u32 size, u32 vCount, VertexAttribute *attrs, ModelStreamType dataType) {
...
void __modelUpdateData(Model *self, void *data, u32 size, u32 vCount) {
...
void r_enableDepth(Renderer *self, bool enabled) {
   if (enabled) {
...
void r_enableAlphaBlending(Renderer *self, bool enabled) {
   if (enabled) {
      glEnable(GL_BLEND);
...
void r_enableWireframe(Renderer *self, bool enabled) {
   if (enabled) {
...
void __r_setUBOData(Renderer *self, UBO *ubo, u32 offset, u32 size, void *data) {
